    ...
    public:
        DriftNode() : Node("drift_node") {
            drift_subscriber = create_subscription<Twist>("drift_cmd_vel", 10,
                [this](Twist::SharedPtr msg) {Subscription_callback(msg);});
        ...
        void Subscription_callback(const Twist::SharedPtr msg) {
            ...
            //I add 1.1 to it so cos wouldn't nullify deacceleration
            double linDeaccel = linSpeed*abs((-cos(angSpeed)+1.1)*2.5)*0.1;
